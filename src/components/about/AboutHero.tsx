  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <motion.div className="absolute top-[35%] left-[20%] w-32 h-32 rounded-full bg-gradient-to-br from-[#4F6BFF]/10 to-transparent blur-xl" animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.7, 0.3] }} transition={{ duration: 8, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" }} />
      </div>
      ...
      <div className="relative h-screen flex flex-col justify-center items-center">
        <motion.div className="container mx-auto px-4 text-center" style={{ opacity: contentOpacity }}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} className="max-w-4xl mx-auto">
            <h1 className="text-[clamp(3rem,8vw,6.5rem)] leading-[1.1] font-display tracking-[-0.03em] text-[#0A2540]">
              ...
              <motion.span
                ...
                onMouseEnter={() => setWavePaused(true)}
                onMouseLeave={() => setWavePaused(false)}
              >
                meets technology
              </motion.span>
            </h1>
          ...
          <motion.div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.4 }}>
            <motion.a href="/about#story" className="group relative overflow-hidden rounded-full bg-[#4F6BFF] px-8 py-4 text-white shadow-lg" whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
