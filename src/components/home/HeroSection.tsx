...
const HeroSection = () => {
  ...
  const [mounted, setMounted] = useState(false); // prevent first-paint blink
  ...
  // Helpers to avoid SSR flash: start visible on SSR, animate on client only
  const initUp = mounted ? { opacity: 0, y: 30 } : { opacity: 1, y: 0 };
  const initUpSm = mounted ? { opacity: 0, y: 20 } : { opacity: 1, y: 0 };
        ...
        <motion.div className="container mx-auto px-4 text-center" style={{ opacity: contentOpacity }}>
          ...
          >
            <div className="text-center">
              ...
              <div className="mt-2 text-sm text-[#505c6e] uppercase tracking-wider">
