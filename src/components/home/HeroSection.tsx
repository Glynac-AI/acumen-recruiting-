...
// tiny helper: layout effect on client, effect on server
...
const HeroSection = () => {
  ...
  // ----- Mount guard: prevent SSR → CSR blink
  const [mounted, setMounted] = useState(false);
  useIsoLayoutEffect(() => setMounted(true), []);

  // ----- Parallax effect (sprung for smoothness)
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start start", "end start"],
  });

  const bgYRaw = useTransform(scrollYProgress, [0, 1], [0, 100]);
  const bgScaleRaw = useTransform(scrollYProgress, [0, 1], [1, 1.15]);
  const contentOpacityRaw = useTransform(scrollYProgress, [0, 0.5], [1, 0]);

  const bgY = useSpring(bgYRaw, { stiffness: 120, damping: 24, mass: 0.6 });
  const bgScale = useSpring(bgScaleRaw, { stiffness: 120, damping: 24, mass: 0.6 });
  const contentOpacity = useSpring(contentOpacityRaw, { stiffness: 120, damping: 26, mass: 0.7 });

  // ----- Cursor follower (rAF throttled)
  const [cursorHovering, setCursorHovering] = useState(false);
  const [hasMouseFrame, setHasMouseFrame] = useState(false);
  ...
  const cursorX = useSpring(mousePosition.x, { damping: 30, stiffness: 200, mass: 0.4 });
  const cursorY = useSpring(mousePosition.y, { damping: 30, stiffness: 200, mass: 0.4 });

  // ----- Wave animation toggle
  const [wavePaused, setWavePaused] = useState(false);
  const toggleWaveAnimation = () => setWavePaused((p) => !p);
  ...
  return (
    ...
    >
      ...
      <AnimatePresence initial={false}>
        {cursorHovering && hasMouseFrame && (
          <motion.div
            className="fixed top-0 left-0 z-50 pointer-events-none mix-blend-difference transform-gpu"
      ...
      <div className="absolute inset-0 overflow-hidden">
        {/* Gradient layer */}
        <motion.div
          className="absolute inset-0 bg-gradient-to-b from-[#f5f8ff] to-white transform-gpu"
          style={{ y: bgY, scale: bgScale, willChange: "transform" }}
          initial={false}
        />

        {/* Dynamic wave (no initial anim on SSR) */}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          initial={false}
          className="absolute top-[15%] right-[10%] w-64 h-64 rounded-full border border-[#4F6BFF]/10 transform-gpu"
        ...
        <motion.div
          initial={false}
          className="absolute bottom-[25%] left-[15%] w-40 h-40 border border-[#4F6BFF]/10 rounded-xl transform-gpu"
        ...
        <motion.div
          initial={false}
          className="absolute top-[35%] left-[20%] w-32 h-32 rounded-full bg-gradient-to-br from-[#4F6BFF]/10 to-transparent blur-xl transform-gpu"
      ...
      <div className="relative h-screen flex flex-col justify-center items-center">
        ...
        >
          <motion.div
            // No "initial" on server; fade in only after mount
          ...
          >
            <motion.a
              href="/contact"
              className="group relative overflow-hidden rounded-full bg-[#4F6BFF] px-8 py-4 text-white shadow-lg transform-gpu"
              whileHover={{ y: -4 }}
              transition={{ duration: 0.2, ease: EASE }}
            >
              <span className="relative z-10 text-base font-medium tracking-wide">Schedule a consultation</span>
              {/* gentle sheen without SSR mismatch */}
              <span className="absolute inset-0 bg-gradient-to-r from-[#4F6BFF] to-[#4F6BFF]/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            </motion.a>

            <motion.a
              href="/services"
              className="group relative rounded-full border border-[#E5E7EB] px-8 py-4 text-[#0A2540] hover:border-[#4F6BFF]/30 transition-colors transform-gpu"
              whileHover={{ y: -4 }}
              transition={{ duration: 0.2, ease: EASE }}
            >
          ...
          >
            <div className="text-center">
              <div className="text-5xl font-light text-[#0A2540]">
                2,000<span className="text-[#4F6BFF]">+</span>
              </div>
              <div className="mt-2 text-sm text-[#505c6e] uppercase tracking-wider">Pre-screened candidates</div>
            </div>

            <div className="h-10 w-px bg-gradient-to-b from-transparent via-[#E5E7EB] to-transparent" />
