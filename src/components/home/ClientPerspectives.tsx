...
const ClientPerspectives: React.FC = () => {
  ...
  // Scroll-based background parallax (subtle & GPU-accelerated)
  ...
  // Auto-advance carousel (paused on hover, reduced-motion, or when tab hidden)
  useEffect(() => {
  ...
  }, [isPaused, prefersReduced, testimonials.length]);
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(to bottom, rgba(245, 248, 255, 0.8), rgba(255, 255, 255, 1))' }}
    >
      {/* Background */}
      <motion.div
        className="absolute inset-0 w-full h-full opacity-40 transform-gpu will-change-transform"
        style={{
          y: backgroundY,
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {!prefersReduced && (
          <>
            <motion.div
              initial={false}
              className="absolute top-[20%] right-[15%] w-64 h-64 rounded-full border border-[#4F6BFF]/10 transform-gpu will-change-transform"
              animate={{ y: [0, -20, 0], rotate: [0, 5, 0], scale: [1, 1.05, 1] }}
              transition={{ duration: 15, repeat: Infinity, repeatType: 'reverse', ease: 'easeInOut' }}
            />
            <motion.div
              initial={false}
              className="absolute bottom-[15%] left-[10%] w-48 h-48 border border-[#6366F1]/10 rounded-full transform-gpu will-change-transform"
      ...
      <div className="container mx-auto px-6 relative z-10">
        {/* Header (client-only initial to prevent flash) */}
        <motion.div
          className="max-w-3xl mx-auto text-center mb-16"
          initial={mounted && !prefersReduced ? { opacity: 0, y: 40 } : false}
        ...
        >
          ...
          >
            Insights from wealth management firms on their experience with Acumen Recruiting
          </motion.p>
        </motion.div>
        ...
        >
          <div className="lg:col-span-4 bg-white p-8 md:p-12 lg:p-16 relative overflow-hidden min-h-[400px] flex items-center rounded-xl shadow-sm">
            ...
            <AnimatePresence mode="wait" initial={false} custom={direction}>
              <motion.div
                key={activeIndex}
                custom={direction}
                initial={mounted && !prefersReduced ? { opacity: 0, y: direction > 0 ? 16 : -16 } : false}
              ...
              >
                <div className="max-w-2xl mx-auto">
                    ...
                    >
                      {(() => {
                        ...
                        return (
                          <>
                            {parts[0]}
                            <motion.span className="relative inline" initial={mounted && !prefersReduced ? { opacity: 0 } : false} animate={{ opacity: 1 }} transition={{ delay: prefersReduced ? 0 : 0.25, duration: prefersReduced ? 0 : 0.5, ease: EASE }}>
                  ...
                  <div className="flex items-center mb-6">
                    <div className="flex mr-3">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <motion.div key={i} initial={mounted && !prefersReduced ? { opacity: 0, scale: 0.95 } : false} animate={{ opacity: 1, scale: 1 }} transition={{ delay: prefersReduced ? 0 : 0.05 + i * 0.06, duration: prefersReduced ? 0 : 0.25, ease: EASE }}>
                          <Star className={`w-4 h-4 mr-1 ${i < testimonials[activeIndex].rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} />
                        </motion.div>
                      ))}
                    </div>
                  ...
                  <motion.div className="flex items-center" initial={mounted && !prefersReduced ? { opacity: 0, y: 8 } : false} animate={{ opacity: 1, y: 0 }} transition={{ delay: prefersReduced ? 0 : 0.25, duration: prefersReduced ? 0 : 0.45, ease: EASE }}>
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground mt-0.5">{testimonials[activeIndex].title}</div>
                    </div>
                  </motion.div>
                </div>
              </motion.div>
              ...
              {testimonials.map((_, index) => (
                ...
                >
                  <motion.div
                    ...
                    initial={false}
                    animate={{ backgroundColor: activeIndex === index ? testimonials[index].color : '#E5E7EB' }}
                    whileHover={{ backgroundColor: activeIndex === index ? testimonials[index].color : '#CBD5E1' }}
                  >
                    {activeIndex === index && (
                      <motion.div
        ...
        >
          ...
          >
            ...
            >
              ...
              {!prefersReduced && (
                <motion.div
                  className="absolute inset-0 bg-gradient-to-r from-ph via-[#6366F1] to-ph transform-gpu will-change-transform"
                  initial={false}
                  whileHover={{ x: 0 }}
                  transition={{ duration: 0.4, ease: EASE }}
                  style={{ x: '100%', backfaceVisibility: 'hidden' }}
                />
              )}
            </motion.a>
          </motion.div>
...
// Simple, non-animated card (kept subtle)
const InsightCard = ({
  title,
  description,
  color,
    ...
    <div className="px-8 py-12 relative group transition-all duration-300 hover:bg-white hover:shadow-md border-r border-b border-gray-100 last:border-r-0 md:last:border-r md:first:border-l-0 md:border-r-0">
      <div className="flex flex-col items-center text-center">
        ...
        <h4 className="text-lg font-medium text-foreground mb-3 relative">
          {title}
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 group-hover:w-full transition-all duration-300" style={{ backgroundColor: color }} />
        </h4>
